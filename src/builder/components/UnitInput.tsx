      ...
      <PopoverTrigger asChild>
        <div className={`flex gap-0.5 ${className}`} style={{ alignItems: 'center', ...style }}>
          ...
          <Select value={unit} onValueChange={(newUnit) => onChange(number ? `${number}${newUnit}` : '')}>
            <SelectTrigger className="w-8 h-6 text-[8px] border-border px-0 justify-start gap-0.5 bg-[#F5F5F5] dark:bg-[#09090b] text-foreground sp-trigger sp-trigger-compact">
              <SelectValue />
            </SelectTrigger>
